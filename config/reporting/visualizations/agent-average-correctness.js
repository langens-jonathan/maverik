// Line chart: Pass rate averaged per agent, one point per agent in `data` (x-axis =
// agent id, y-axis = the average of pass rate across that agent's selected runs).
...
export default function (container, data, { d3, halfWidth }) {
  ...
  for (const r of data) {
    const value = r.summary.passRate;
  ...
  if (points.length === 0) {
    container.textContent = "No data for Pass rate.";
    return;
  }

  const width = halfWidth ?? 560;
  const height = 280;
  const margin = { top: 16, right: 20, bottom: 72, left: 56 };

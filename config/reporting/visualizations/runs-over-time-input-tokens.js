// Line chart: Avg input tokens over time, one point per SuiteRunRecord in `data`, x-axis = the
...
export default function (container, data, { d3 }) {
  ...
  const points = data
    .map((r) => ({ x: new Date(r.timestamp), y: r.summary.avgInputTokens, agentId: r.agentId, suiteId: r.suiteId }))

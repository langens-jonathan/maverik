// Line chart: Avg output tokens over time, one point per SuiteRunRecord in `data`, x-axis = the
...
export default function (container, data, { d3, halfWidth }) {
  ...
  const points = data
    .map((r) => ({ x: new Date(r.timestamp), y: r.summary.avgOutputTokens, agentId: r.agentId, suiteId: r.suiteId }))

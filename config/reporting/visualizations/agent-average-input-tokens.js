// Line chart: Avg input tokens averaged per agent, one point per agent in `data` (x-axis =
// agent id, y-axis = the average of avg input tokens across that agent's selected runs).
...
export default function (container, data, { d3, halfWidth }) {
  ...
  for (const r of data) {
    const value = r.summary.avgInputTokens;
  ...
  if (points.length === 0) {
    container.textContent = "No data for Avg input tokens.";
    return;
  }

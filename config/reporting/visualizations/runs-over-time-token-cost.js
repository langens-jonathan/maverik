// Line chart: Token cost ($) over time, one point per SuiteRunRecord in `data`, x-axis = the
...
export default function (container, data, { d3, halfWidth }) {
  ...
  const points = data
    .map((r) => ({ x: new Date(r.timestamp), y: r.summary.estCostTotal, agentId: r.agentId, suiteId: r.suiteId }))

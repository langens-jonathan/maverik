// Line chart: Overall cost ($) over time, one point per SuiteRunRecord in `data`, x-axis = the
...
export default function (container, data, { d3, halfWidth }) {
  ...
  const points = data
    .map((r) => ({ x: new Date(r.timestamp), y: r.summary.estOverallCostTotal, agentId: r.agentId, suiteId: r.suiteId }))
    .filter((p) => p.y !== null && p.y !== undefined && !Number.isNaN(p.x.getTime()))
    .sort((a, b) => a.x - b.x);

  if (points.length === 0) {
    container.textContent = "No data for Overall cost ($).";
    return;
  }

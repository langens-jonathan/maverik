// Beeswarm: per-case duration spread, one lane per agent, across every selected run — the direct
// counterpart to cost-per-question-distribution.js, for the same reason: agent-average-duration.js
// and runs-over-time-duration.js only ever show an average, which hides whether an agent's
// duration is consistent or has a long tail of slow outlier questions. See ../README.md for the
// function contract.
//
...
// charts/distributionStrip.js is built on. Low-n honesty follows the same rule as
// distributionStrip.js/cost-per-question-distribution.js: real per-case dots, never a fabricated
// smooth distribution, and a mean tick drawn separately from the dots it summarizes.
export default function (container, data, { d3, halfWidth, chartKit }) {
  ...
  for (const c of cases) {
    ...
    byAgent.get(c.agentId).push(c.durationMs);
  }

  const agentIds = [...byAgent.keys()].sort((a, b) => a.localeCompare(b));
  if (agentIds.length === 0) {
    chartKit.showEmptyState(container, "No per-case duration data for the selected runs.");
    return;
  }

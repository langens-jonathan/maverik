// Line chart: Token cost ($) averaged per agent, one point per agent in `data` (x-axis =
// agent id, y-axis = the average of token cost ($) across that agent's selected runs).
...
export default function (container, data, { d3 }) {
  ...
  for (const r of data) {
    const value = r.summary.estCostTotal;
  ...
  if (points.length === 0) {
    container.textContent = "No data for Token cost ($).";
    return;
  }
